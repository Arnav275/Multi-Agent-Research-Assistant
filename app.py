...
FIX 1: load_dotenv() called at the very top before any imports that need keys.
FIX 2: sys.path set to project root so both core.* and ingestion.* resolve.
FIX 3: run_query imported at top level after dotenv loads — not inside if block.
FIX 4: Sidebar key saving now resets the pipeline cache so new keys take effect.
"""
...
# ✅ FIX 1: Set project root on path FIRST, before any local imports
PROJECT_ROOT = Path(__file__).parent.parent
sys.path.insert(0, str(PROJECT_ROOT))

# ✅ FIX 2: Load .env BEFORE importing any agents or graph
...
with st.sidebar:
    ...
    if st.button("💾 Save & Apply Keys", use_container_width=True):
        ...
        # ✅ FIX 3: Reset pipeline cache so next query picks up new keys
...
if run_btn and query.strip():
    # ✅ FIX 4: Import run_query here (after dotenv loaded) not at file top
